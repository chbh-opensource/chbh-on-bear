...
a **device-to-head transformation is calculated for each acquisition session**. This mathematical alignment **relies on 
five (*or four in CHBH MEG usage*) coils placed on the participant's head**.<br /> The positions of these coils are **measured in two 
...
!!! Info "LPA and RPA, used in CHBH, are known as LHS and RHS - Left and Right Helix-Scalp junction - in other naming conventions."
...
- The **head shape** (**additional (200+) points on the scalp to improve co-registration**).

!!! Note "The head coordinate frame measures the point’s position in metres."

## HPI Measurement

A **HPI** (**H**ead **P**osition **I**ndication, sometimes **cHPI** for ***continuous*** head position indication) measurement is **always performed before a new acquisition with a participant**. 
...
!!! Note
	**Only 3 coils are needed to estimate the head position**, 4 coils ensures that they are not coplanar. MEGIN opted for 5 coils as a safety margin to ensure a robust estimation.<br /> 
	**Coil sets with broken coils can still be used**, as long as **at least 3 coils are usable** - **coil colour and placement on the head doesn't matter**.

The **HPI measurement at the beginning of the recording** is used to **estimate a single device to head transformation**, stored in the **```raw.info["dev_head_t"]```** field of a **[Raw](https://mne.tools/stable/generated/mne.io.Raw.html#mne.io.Raw)** object.


!!! Note 
	In **[MNE-Python](https://mne.tools/stable/index.html)**, the the **device to head transformation** is **stored** in a **[Transform](https://mne.tools/stable/generated/mne.transforms.Transform.html#mne.transforms.Transform)** object **representing a 4x4 affine transformation**.
...
!!! Tip
	...
	**[Fine-Calibration files / cross-talk correction file](../../meg/analysis/maxfiltercalfiles.md)** are available to download.
...
!!! Note
	**Whatever software is used, the cHPI signal should be filtered out before any analysis takes place**.<br />
	In **[MNE-Python](https://mne.tools/stable/index.html)**), this is done through **[filter_chpi()](https://mne.tools/stable/generated/mne.chpi.filter_chpi.html#mne.chpi.filter_chpi)**.<br />
	This function is not a simple notch filter as the cHPI signals are generally not stationary because any head movements act like amplitude modulators. Therefore an iterative fitting method is used to remove the cHPI signal.

!!! Tip
	**[MNE-Python](https://mne.tools/stable/index.html)** has command-line tools which can be accessed with:<br>
	**```$ mne <command> <options>```**<br />
	Entering the command **```$ mne```** will return a list of available commands.<br />
	The commands **```mne what```** and **```mne show_fiff```** can be used to inspect the content of a FIFF file.
