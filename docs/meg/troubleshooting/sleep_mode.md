...
**1) <span style="color:blue">Press ```Ctrl-Alt-Backspace```**
...
- This restarts the whole Desktop GUI session and will kill all open applications/logout the ***meguser*** account. The login GUI screen should then reappear.
	- Any collected data ***may*** be recoverable. Try the **procedure on page 44** of the **[Data Acquisition User Manual](https://canvas.bham.ac.uk/courses/81128/files/folder/MEG?preview=21940766)** (***Section 6.4: "Rescuing data after a crash"***).

***<span style="color:green">NOT recommended ideally, but if necessary ...</span>***

**2) <span style="color:blue">Press the DACQ Power button *lightly!!!*</span>**  **<span style="color:red">Pressing *too* hard will cause a reboot of the Console!!!</span>**
...
!!! Note "```Ctrl-Alt-Backspace``` doesn't always work - DACQ screen remains blank.<br /> If so, then the DACQ/Console will need to be rebooted (may have also occured unintentionally if the Power button was pressed too hard.<br />```Ctrl-Alt-Delete``` should also cause a reboot, or perform Point 2) "*Restart Computer*" shown above (*prompt #3*)."

**3) <span style="color:blue">Reboot the DACQ/Console.</span>**
...
then check to see if the **gantry position is now detected correctly** in **Acquisition** by **left-clicking** on the **Gantry position "Change" button**.<br /> If still not detected correctly, then ...
	...
	- Then perform another **[RAP](../../meg/acquisition/rap.md)**, then **[load the latest tuning file](../../meg/acquisition/restart_acquisition.md/#loading-a-tuning-file)**.
